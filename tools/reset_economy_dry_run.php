...
$options = getopt('', ['season-id::', 'apply-migration', 'migration-file::']);
$db = Database::getInstance();

$requestedSeasonId = isset($options['season-id']) ? (int)$options['season-id'] : null;
$seasonId = resolveSeasonId($db, $requestedSeasonId);
if ($seasonId === null || $seasonId <= 0) {
    fwrite(STDERR, "Unable to resolve target season. Pass --season-id=<id>.\n");
    exit(1);
}

$pre = auditSeasonReset($db, $seasonId);
...
$migrationFile = $options['migration-file'] ?? 'migration_20260402d_reset_season_economy_preserve_auth.sql';
...
$post = auditSeasonReset($db, $seasonId);
printJson('POST-CHECK AUDIT', $post);

$checks = evaluatePostChecks($post, $seasonId);
