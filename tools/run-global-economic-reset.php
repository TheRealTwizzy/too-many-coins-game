<?php
/**
 * Run global economic reset (preserve accounts/auth) and verification via PDO.
 *
 * Usage:
 *   php tools/run-global-economic-reset.php
 *   php tools/run-global-economic-reset.php --verify-only
 *   php tools/run-global-economic-reset.php --no-verify
 */
...
function printUsage(): void {
    echo "Usage:\n";
    echo "  php tools/run-global-economic-reset.php\n";
    echo "  php tools/run-global-economic-reset.php --verify-only\n";
    echo "  php tools/run-global-economic-reset.php --no-verify\n";
}
...
function runReset(PDO $pdo): void {
    $sqlPath = __DIR__ . '/reinitialize-global-economy-preserve-accounts.sql';
    ...
    $pdo->exec($sql);
    echo "[ok] global economic reset script applied\n";
}
...
function runVerification(PDO $pdo): void {
    ...
    echo "[verify] global economy wipe\n";
    $globalEconomy = fetchRows(
        $pdo,
        "SELECT
            COALESCE((SELECT SUM(global_stars) FROM players), 0) AS total_global_stars,
            COALESCE((SELECT SUM(global_stars_fractional_fp) FROM players), 0) AS total_global_stars_fractional_fp,
            (SELECT COUNT(*) FROM player_cosmetics) AS player_cosmetics_count"
    );
    echo json_encode($globalEconomy, JSON_PRETTY_PRINT) . "\n";
