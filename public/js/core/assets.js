...
/**
 * Every slot is filled. The art is committed SVG under /assets, generated by
 * tools/generate_assets.mjs — deterministic, diff-reviewable, same-origin (so
 * the `img-src 'self'` CSP is satisfied), and resolution-independent, which is
 * why no slot needs an @2x companion.
 *
 * The placeholders are kept deliberately. They are the fallback if a file ever
 * fails to load, and they document what each slot is meant to depict.
 */
const img = (src, w, h, extra = {}) => ({ kind: 'image', src: `/assets/${src}`, w, h, ...extra });
const sprite = (src, w, h, frames, fps) => ({ kind: 'sprite', src: `/assets/${src}`, w, h, frames, fps, loop: false });

export const registry = {
    // --- navigation ---
    'nav-home':     { placeholder: '★',      art: img('nav-home.svg', 24, 24) },
    'nav-seasons':  { placeholder: '\u{1F3C6}',   art: img('nav-seasons.svg', 24, 24) },
    'nav-ranks':    { placeholder: '\u{1F3C5}',   art: img('nav-ranks.svg', 24, 24) },
    'nav-chat':     { placeholder: '\u{1F4AC}',   art: img('nav-chat.svg', 24, 24) },
    'nav-shop':     { placeholder: '\u{1F48E}',   art: img('nav-shop.svg', 24, 24) },
    'nav-staff':    { placeholder: '\u{1F6E1}',   art: img('nav-staff.svg', 24, 24) },   // staff/admin rail entry (staff viewers only)
    'nav-family':   { placeholder: '⁂',      art: img('nav-family.svg', 24, 24) },   // family panel rail entry (families-enabled seasons)

    // --- currencies ---
    // The two star currencies are deliberately different silhouettes — a
    // 4-point hollow mark for seasonal, an 8-point solid for global — so they
    // stay distinguishable at HUD size where colour alone would not carry it.
    'coin':         { placeholder: '\u{1FA99}',   art: img('coin.svg', 20, 20) },
    'star-season':  { placeholder: '✦',      art: img('star-season.svg', 20, 20) },
    'star-global':  { placeholder: '✴',      art: img('star-global.svg', 20, 20) },
    'sigil':        { placeholder: '◇',      art: img('sigil.svg', 20, 20) },

    // --- sigil families ---
    // These carry identity a player learns once, so they are the slots where
    // custom art pays off most and where the placeholders read worst. Each
    // ships as a white silhouette flagged `tintable`, so assets.js masks it and
    // paints the family's own CSS token through it: the shape is the identity,
    // and all four themes recolour it for free.
    'family-yield':   { placeholder: '●', tint: 'var(--family-yield)',   art: img('family-yield.svg', 28, 28, { tintable: true }) },
    'family-time':    { placeholder: '◔', tint: 'var(--family-time)',    art: img('family-time.svg', 28, 28, { tintable: true }) },
    'family-ward':    { placeholder: '◈', tint: 'var(--family-ward)',    art: img('family-ward.svg', 28, 28, { tintable: true }) },
    'family-larceny': { placeholder: '◆', tint: 'var(--family-larceny)', art: img('family-larceny.svg', 28, 28, { tintable: true }) },
    'family-market':  { placeholder: '▲', tint: 'var(--family-market)',  art: img('family-market.svg', 28, 28, { tintable: true }) },
    'family-sight':   { placeholder: '◎', tint: 'var(--family-sight)',   art: img('family-sight.svg', 28, 28, { tintable: true }) },
    'family-wild':    { placeholder: '✳', tint: 'var(--family-wild)',    art: img('family-wild.svg', 28, 28, { tintable: true }) },
    ...
    'payout-burst': { placeholder: null, art: sprite('payout-burst.svg', 96, 96, 12, 24), wants: 'sprite' },
    'sigil-drop':   { placeholder: null, art: sprite('sigil-drop.svg', 128, 128, 16, 24), wants: 'sprite' },
    'theft-strike': { placeholder: null, art: sprite('theft-strike.svg', 128, 128, 14, 24), wants: 'sprite' },
    'freeze-lock':  { placeholder: null, art: sprite('freeze-lock.svg', 96, 96, 10, 20), wants: 'sprite' },
    'lockin-seal':  { placeholder: null, art: sprite('lockin-seal.svg', 192, 192, 24, 24), wants: 'sprite' },

    // --- ui chrome ---
    'bell':           { placeholder: '\u{1F514}', art: img('bell.svg', 24, 24) },

    // --- states ---
    'state-idle':     { placeholder: '⏳', art: img('state-idle.svg', 24, 24) },
    'state-blackout': { placeholder: '⚠', art: img('state-blackout.svg', 24, 24) },
    'state-frozen':   { placeholder: '❄', art: img('state-frozen.svg', 24, 24) },
};
