# Economy Tuning Candidates (v3)

Generated: 2026-04-10 22:27:26 UTC
...
## Regression Mitigation Strategy (v3)

All v1 packages were REJECTED on all seeds due to regression flags.
v3 applies a stability-first mitigation profile:

1. **Minimal hoarding drain deltas** — reduce archetype-rank reshuffling risk
2. **Stronger lock-in support** — larger `starprice_reactivation_window_ticks` increase and stronger affordability bias reduction
3. **Smoother expiry recovery** — larger `starprice_max_downstep_fp` increase to prevent price-stuck expiry
4. **Controlled multi-axis tuning** — preserve anti-hoarding intent without single-axis overcorrection

| Regression Flag | v1 Rate | v3 Target |
|---|---|---|
...
|---|--------|---------|----------|------|----------|---------|
| C1 | `hoarding_tier2_rate_hourly_fp` | 500 | 520 | LOW | hoarding_sink | B11 |
| C2 | `hoarding_tier3_rate_hourly_fp` | 1000 | 1050 | LOW | hoarding_sink | B11 |
| C3 | `hoarding_idle_multiplier_fp` | 1250000 | 1262500 | LOW | hoarding_sink | B11 |
| C4 | `starprice_reactivation_window_ticks` | 75 | 86 | LOW | star_pricing | regression_counterweight |
| C5 | `market_affordability_bias_fp` | 1000000 | 940000 | LOW | star_pricing | regression_counterweight |
| C6 | `starprice_max_downstep_fp` | 12000 | 13440 | LOW | star_pricing | regression_counterweight |
...
| # | Target | Current | Proposed | Risk | Mechanic | Finding |
|---|--------|---------|----------|------|----------|---------|
| C7 | `hoarding_tier2_rate_hourly_fp` | 500 | 540 | LOW | hoarding_sink | B11 |
| C8 | `hoarding_tier3_rate_hourly_fp` | 1000 | 1100 | LOW | hoarding_sink | B11 |
| C9 | `hoarding_idle_multiplier_fp` | 1250000 | 1300000 | LOW | hoarding_sink | B11 |
...
| C12 | `target_spend_rate_per_tick` | 15 | 14 | LOW | boost_economy | B10 |
| C13 | `hoarding_min_factor_fp` | 100000 | 108000 | LOW | hoarding_sink | B10 |
| C14 | `hoarding_window_ticks` | 17280 | 16243 | LOW | hoarding_sink | B12 |
| C15 | `starprice_reactivation_window_ticks` | 75 | 93 | MEDIUM | star_pricing | regression_counterweight |
| C16 | `market_affordability_bias_fp` | 1000000 | 900000 | LOW | star_pricing | regression_counterweight |
| C17 | `starprice_max_downstep_fp` | 12000 | 14640 | MEDIUM | star_pricing | regression_counterweight |
...
| # | Target | Current | Proposed | Risk | Mechanic | Finding |
|---|--------|---------|----------|------|----------|---------|
| C18 | `hoarding_tier2_rate_hourly_fp` | 500 | 570 | LOW | hoarding_sink | B11 |
| C19 | `hoarding_tier3_rate_hourly_fp` | 1000 | 1160 | MEDIUM | hoarding_sink | B11 |
| C20 | `hoarding_idle_multiplier_fp` | 1250000 | 1350000 | LOW | hoarding_sink | B11 |
...
| C23 | `target_spend_rate_per_tick` | 15 | 13 | LOW | boost_economy | B10 |
| C24 | `hoarding_min_factor_fp` | 100000 | 115000 | LOW | hoarding_sink | B10 |
| C25 | `hoarding_window_ticks` | 17280 | 15552 | LOW | hoarding_sink | B12 |
| C26 | `starprice_reactivation_window_ticks` | 75 | 101 | HIGH | star_pricing | regression_counterweight |
| C27 | `market_affordability_bias_fp` | 1000000 | 850000 | LOW | star_pricing | regression_counterweight |
| C28 | `starprice_max_downstep_fp` | 12000 | 15840 | HIGH | star_pricing | regression_counterweight |
...
```json
{
    "hoarding_tier2_rate_hourly_fp": 540,
    "hoarding_tier3_rate_hourly_fp": 1100,
    "hoarding_idle_multiplier_fp": 1300000,
    ...
    "target_spend_rate_per_tick": 14,
    "hoarding_min_factor_fp": 108000,
    "hoarding_window_ticks": 16243,
    "starprice_reactivation_window_ticks": 93,
    "market_affordability_bias_fp": 900000,
    "starprice_max_downstep_fp": 14640
}
```

### `tuning-aggressive-v3`
...
```json
{
    "hoarding_tier2_rate_hourly_fp": 570,
    "hoarding_tier3_rate_hourly_fp": 1160,
    ...
    "hoarding_window_ticks": 15552,
    "starprice_reactivation_window_ticks": 101,
    "market_affordability_bias_fp": 850000,
    "starprice_max_downstep_fp": 15840
}
```
...
2. Run Phase D verification sweep:
   ...
   php scripts/simulate_policy_sweep.php \
     --seed=tuning-verify-v3 \
     --scenarios=tuning-conservative-v3,tuning-balanced-v3,tuning-aggressive-v3 \
