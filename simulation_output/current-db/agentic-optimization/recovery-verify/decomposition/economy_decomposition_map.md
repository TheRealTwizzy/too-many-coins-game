...
- Coupling harness families: hoarding_pressure_imbalance
- Search limits: max_candidates=4, max_accepted=1
- Owned parameters:
- Local metrics:
...
- Coupling harness families: lock_in_down_but_expiry_dominance_up
- Search limits: max_candidates=4, max_accepted=1
- Owned parameters:
...
- Coupling harness families: boost_underperformance
- Search limits: max_candidates=4, max_accepted=1
- Owned parameters:
...
- Coupling harness families: hoarding_pressure_imbalance, skip_rejoin_exploit_worsened
- Search limits: max_candidates=4, max_accepted=1
- Owned parameters:
  - `market_affordability_bias_fp` (multiply)
- Local metrics:
...
- Coupling harness families: star_affordability_pricing_instability
- Search limits: max_candidates=4, max_accepted=1
- Owned parameters:
  - `base_ubi_active_per_tick` (multiply)
...
- Promotion gates: Tier1 score >= 0.18, Tier2 global delta >= -0.08, Tier3 global delta >= 0
- Coupling harness families: skip_rejoin_exploit_worsened
- Search limits: max_candidates=4, max_accepted=1
- Owned parameters:
  - `hoarding_min_factor_fp` (multiply)
  - `base_ubi_active_per_tick` (multiply)
...
- `tier1_hoarding` (tier1): Cheap local screening for anti-safe-strategy pressure. [slice=1440 ticks, blackout=360]
- `tier1_blackout` (tier1): Blackout and lock-in pressure micro-harness. [slice=1440 ticks, blackout=480]
- `tier1_boost` (tier1): Boost-focused local screening with reduced cohorts. [slice=1440 ticks, blackout=360]
- `tier1_concentration` (tier1): Short lifecycle concentration stress harness. [slice=1800 ticks, blackout=450]
- `tier1_sigil` (tier1): Sigil acquisition/combine/theft local harness. [slice=1200 ticks, blackout=300]
- `tier1_onboarding` (tier1): Onboarding-only phase-limited harness. [slice=960 ticks, blackout=240]
- `tier1_star_pricing` (tier1): Star market and purchasing local harness. [slice=1200 ticks, blackout=300]
- `tier1_expiry` (tier1): Expiry pressure and lock-in gradient harness. [slice=1440 ticks, blackout=420]
- `tier1_lockin` (tier1): Lock-in incentive and timing harness. [slice=1200 ticks, blackout=360]
- `tier1_retention` (tier1): Repeated-season retention proxy harness. [slice=1440 ticks, blackout=360]
- `coupling_hoarding` (tier1): Extra-cheap hoarding pressure harness for early promotion filtering. [slice=960 ticks, blackout=240]
- `coupling_lockin_expiry` (tier1): Extra-cheap lock-in vs expiry coupling harness. [slice=720 ticks, blackout=180]
- `coupling_boost` (tier1): Extra-cheap boost viability harness. [slice=720 ticks, blackout=180]
- `coupling_star_pricing` (tier1): Extra-cheap star affordability and price stability harness. [slice=720 ticks, blackout=180]
- `coupling_skip_rejoin` (tier1): Extra-cheap repeat-season skip/rejoin exploit harness. [slice=960 ticks, blackout=240]
- `tier2_integration` (tier2): Cross-subsystem integration validation. [slice=4320 ticks, blackout=1080]
- `tier3_full` (tier3): Full lifecycle acceptance gate (agentic screening profile). Uses reduced cost vs promotion-ladder tier3 to prevent PHP timeout. Official promotion uses SweepComparatorCampaignRunner qualification profile as the real final gate. [slice=1440 ticks, blackout=360]
